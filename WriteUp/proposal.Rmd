...
* Visualize the time series data such that users could compare sales values across time along differt categorical attributes.
...
Based on grouping generated the user could then inspect the groups to deep dive into similarity patterns.
...
- Clustering is perform along geographic attribute, then a map and location of the groups would be displayed.

![](imgs/clusteringresults.png)

**Time Series Results Deep Dive**:

Lastly the user to return to the data through time-series line graphs and day of the week cyclic plots to deep dive into specific trends
